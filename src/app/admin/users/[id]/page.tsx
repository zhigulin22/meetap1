...
import { ArrowLeft, Shield, UserX, UserCheck, MessageSquareWarning, EyeOff, Eye, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="min-h-screen space-y-4 p-4">
      ...
      <Card className={justUpdated ? "ring-1 ring-cyan/40" : undefined}>
        <CardHeader><CardTitle>{u.name} · {maskPhone(u.phone)} · {u.country ?? "—"}</CardTitle></CardHeader>
        <CardContent className="space-y-3 text-sm">
          ...
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="secondary" disabled={busy !== null} onClick={() => act(u.message_limited ? "unlimit_messaging" : "limit_messaging")}>{busy === (u.message_limited ? "unlimit_messaging" : "limit_messaging") ? "Applying..." : <><MessageSquareWarning className="mr-1 h-4 w-4" />{u.message_limited ? "Unlimit messaging" : "Limit messaging"}</>}</Button>
            <Button size="sm" variant="secondary" disabled={busy !== null} onClick={() => act(u.shadow_banned ? "unshadowban" : "shadowban")}>{busy === (u.shadow_banned ? "unshadowban" : "shadowban") ? "Applying..." : <>{u.shadow_banned ? <Eye className="mr-1 h-4 w-4" /> : <EyeOff className="mr-1 h-4 w-4" />}{u.shadow_banned ? "Unshadowban" : "Shadowban"}</>}</Button>
            <Button size="sm" variant={u.is_blocked ? "secondary" : "danger"} disabled={busy !== null} onClick={() => act(u.is_blocked ? "unblock" : "block")}>{busy === (u.is_blocked ? "unblock" : "block") ? "Applying..." : <>{u.is_blocked ? <UserCheck className="mr-1 h-4 w-4" /> : <UserX className="mr-1 h-4 w-4" />}{u.is_blocked ? "Unblock" : "Block"}</>}</Button>
            <Button size="sm" disabled={busy !== null} onClick={() => act("mark_safe")}>{busy === "mark_safe" ? "Applying..." : <><Shield className="mr-1 h-4 w-4" />Mark safe</>}</Button>
          </div>
          {actionNote ? (
