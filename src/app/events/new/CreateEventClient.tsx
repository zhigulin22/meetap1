...
function CreateEventPageInner() {
  ...
  async function createOrUpdateDraftForSubmit() {
    const draftRes = await api<{ id: string }>("/api/events/draft", {
      method: "POST",
      body: JSON.stringify({
        event_id: draftId ?? undefined,
      ...
      <div className="mx-auto w-full max-w-3xl">
        ...
        {error ? (
          <div className="mb-3 rounded-xl border border-[rgb(var(--danger-rgb)/0.24)] bg-[rgb(var(--danger-rgb)/0.08)] px-3 py-2 text-xs text-[rgb(var(--danger-rgb))]">
            ...
            {validationErrors.length ? (
              <ul className="mt-2 list-disc pl-4 text-[11px] text-[rgb(var(--danger-rgb))]">
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button variant="secondary" onClick={onSaveDraft} disabled={loading || !draftReady}>
...
export default CreateEventPageInner;
