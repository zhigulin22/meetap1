...
export async function GET(req: Request) {
  try {
    ...
    const block = await getMetricsBlock("content", fromISO, toISO, userIds);
    return ok({ kind: "content", range: { from: fromISO, to: toISO, segment: parsed.data.segment }, ...block });
  } catch (error) {
    return adminRouteError("/api/admin/metrics/content", error);
  }
}
