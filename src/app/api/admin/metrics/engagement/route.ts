...
export async function GET(req: Request) {
  try {
    ...
    const block = await getMetricsBlock("engagement", fromISO, toISO, userIds);
    return ok({ kind: "engagement", range: { from: fromISO, to: toISO, segment: parsed.data.segment }, ...block });
  } catch (error) {
    return adminRouteError("/api/admin/metrics/engagement", error);
  }
}
