...
export async function GET(req: Request) {
  try {
    ...
    const block = await getMetricsBlock("ai", fromISO, toISO, userIds);
    return ok({ kind: "ai", range: { from: fromISO, to: toISO, segment: parsed.data.segment }, ...block });
  } catch (error) {
    return adminRouteError("/api/admin/metrics/ai", error);
  }
}
