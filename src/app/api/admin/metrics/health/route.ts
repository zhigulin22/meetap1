...
export async function GET(req: Request) {
  try {
    ...
    const block = await getMetricsBlock("health", fromISO, toISO, userIds);
    return ok({ kind: "health", range: { from: fromISO, to: toISO, segment: parsed.data.segment }, ...block });
  } catch (error) {
    return adminRouteError("/api/admin/metrics/health", error);
  }
}
