...
export async function GET(req: Request) {
  try {
    ...
    const block = await getMetricsBlock("social", fromISO, toISO, userIds);
    return ok({ kind: "social", range: { from: fromISO, to: toISO, segment: parsed.data.segment }, ...block });
  } catch (error) {
    return adminRouteError("/api/admin/metrics/social", error);
  }
}
