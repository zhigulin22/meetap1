...
export async function GET(req: Request) {
  try {
    ...
    const block = await getMetricsBlock("safety", fromISO, toISO, userIds);
    return ok({ kind: "safety", range: { from: fromISO, to: toISO, segment: parsed.data.segment }, ...block });
  } catch (error) {
    return adminRouteError("/api/admin/metrics/safety", error);
  }
}
