...
export function KpiDrilldownDrawer({
  ...
  return (
    <>
      ...
      <aside className={`fixed right-0 top-0 z-50 h-screen w-full max-w-[560px] border-l border-border bg-surface p-4 shadow-[0_0_80px_rgba(0,0,0,0.45)] transition-transform ${open ? "translate-x-0" : "translate-x-full"}`}>
        ...
        {!loading && data ? (
          <div className="space-y-4 overflow-y-auto pb-16">
            <div className="rounded-2xl border border-border bg-surface2/70 p-3 text-sm">
              ...
              <p>Всего событий: <strong>{Number(data.total_count ?? 0)}</strong></p>
              <p>Уникальных пользователей: <strong>{Number(data.unique_users ?? 0)}</strong></p>
              ...
              <>
                ...
                <section className="rounded-2xl border border-warning/40 bg-warning/10 p-3">
                  ...
                  <div className="max-h-40 space-y-1 overflow-auto text-xs">
                    {topPeriod.length ? topPeriod.map((r) => <p key={r.event_name} className="flex justify-between"><span>{r.event_name}</span><strong>{r.count}</strong></p>) : <p className="text-muted">Нет событий</p>}
                  </div>
                </section>
              </>
            ) : null}
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <section className="rounded-2xl border border-border bg-surface2/70 p-3">
                ...
                <div className="max-h-36 space-y-1 overflow-auto text-xs">
                  {data.breakdown_by_week?.length ? data.breakdown_by_week.map((r) => <p key={r.week_start} className="flex justify-between"><span>{r.week_start}</span><strong>{r.value}</strong></p>) : <p className="text-muted">Нет данных</p>}
                </div>
                <div className="mt-2 space-y-1">
              ...
              <section className="rounded-2xl border border-border bg-surface2/70 p-3">
                ...
                <div className="max-h-36 space-y-1 overflow-auto text-xs">
                  {data.breakdown_by_month?.length ? data.breakdown_by_month.map((r) => <p key={r.month} className="flex justify-between"><span>{r.month}</span><strong>{r.value}</strong></p>) : <p className="text-muted">Нет данных</p>}
                </div>
                <div className="mt-2 space-y-1">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="secondary" onClick={onAutoMap} disabled={autoMapLoading} className="active:scale-[0.98] transition-transform">
                {autoMapLoading ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Wand2 className="mr-1 h-4 w-4" />}Auto-map event names
              </Button>
              <Button variant="secondary" onClick={onCreateExperiment} disabled={experimentLoading} className="active:scale-[0.98] transition-transform">
                {experimentLoading ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <FlaskConical className="mr-1 h-4 w-4" />}Создать эксперимент
              </Button>
